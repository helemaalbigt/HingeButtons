...
sampler2D _FingerMask;
float4 _FingerMask_ST;
//float _EdgeColorBlend;
float _ThumbColorBlend;
float _IndexColorBlend;
float _MiddleColorBlend;
//fixed4 _EdgeColor;
fixed4 _ThumbColor;
fixed4 _IndexColor;
fixed4 _MiddleColor;
//float _EdgeAlphaBlend;
float _ThumbAlphaBlend;
float _IndexAlphaBlend;
float _MiddleAlphaBlend;
...
#endif
	//float edge = tex2D(_FingerMask, TRANSFORM_TEX(uv, _FingerMask)).a * _EdgeColorBlend;
	float thumb = tex2D(_FingerMask, TRANSFORM_TEX(uv, _FingerMask)).r * _ThumbColorBlend;
	float index = tex2D(_FingerMask, TRANSFORM_TEX(uv, _FingerMask)).g * _IndexColorBlend;
	float middle = tex2D(_FingerMask, TRANSFORM_TEX(uv, _FingerMask)).b * _MiddleColorBlend;
#if TIPADJUSTMENT_OFF
	//c.rgb = lerp(c.rgb, _EdgeColor, edge);
	c.rgb = lerp(c.rgb, _ThumbColor, thumb);
	c.rgb = lerp(c.rgb, _IndexColor, index);
	c.rgb = lerp(c.rgb, _MiddleColor, middle);
#endif
...
#else
	float alpha = _Alpha * c.a * tex2D(_AlphaMask, TRANSFORM_TEX(uv, _AlphaMask)).r * IN.vertColor.a * ComputeMask(MASK_INPUTS, _BaseMaskType, _BaseMaskParameters, _BaseMaskAxis);
	//edge *= _EdgeAlphaBlend;
	thumb *= _ThumbAlphaBlend;
	index *= _IndexAlphaBlend;
	middle *= _MiddleAlphaBlend;
	//alpha = alpha * (1.0 - edge) + _EdgeColor.a * edge;
	alpha = alpha * (1.0 - thumb) + _ThumbColor.a * thumb;
	alpha = alpha * (1.0 - index) + _IndexColor.a * index;
	o.Alpha = alpha * (1.0 - middle) + _MiddleColor.a * middle;
#endif
